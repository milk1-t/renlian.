...
def build_model(num_classes: int, variant: str = 'large', pretrained: bool = True) -> nn.Module:
    if variant == 'large':
...
def plot_per_class_acc(y_true, y_pred, classes, out_dir):
    ...
    plt.figure(figsize=(8, 5))
    bars = plt.bar(classes, per_class_acc, color='seagreen')
    plt.ylim(0, 1)
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='runs/mobilenetv3')
    parser.add_argument('--variant', type=str, default='large', choices=['large', 'small'])
    ...
    print(f'Variant: MobileNetV3-{args.variant}')
    ...
    model = build_model(num_classes=len(classes), variant=args.variant,
